        ...
        *,
        ...
        if len(observation_times_ns) >= 2:
            ...
            if elapsed_ns > 0:
                ...
                if now_ns - observation_times_ns[-1] <= stale_after_ns:
                    actual_fps = (
        ...
        return CaptureMetricsSnapshot(
            actual_fps=actual_fps,
            latest_latency_ms=latencies_ns[-1] / _NANOSECONDS_PER_MILLISECOND,
            average_latency_ms=(fmean(latencies_ns) / _NANOSECONDS_PER_MILLISECOND),
            p95_latency_ms=(
