...
class CaptureSession:
    """Own one capture backend and run its full lifecycle on one worker thread.

    The UI may only request cancellation and consume the bounded queues. It never
    receives the backend instance, preventing accidental cross-thread lifecycle
    calls. A requested backend is constructed exactly once; failures are surfaced
    instead of triggering a fallback to a different backend.
    """
    ...
    def start(self) -> None:
        """Start this single-use session without constructing a backend on the UI thread."""
    ...
    def request_stop(self) -> None:
        """Request cooperative cancellation; no backend method is called here."""

        self.stop_event.set()

    def join(self, timeout: float | None = None) -> bool:
        """Wait for the worker and return whether it has terminated."""
    ...
    def _run(self) -> None:
        ...
        try:
            # Resolve the production factory in the worker so construction and
            # every lifecycle operation share this exact thread.
    ...
    def _capture_loop(self, backend: CaptureBackend) -> None:
        interval_ns = (
            ...
            else int(1_000_000_000 / self.target_fps)
        )
        ...
        while not self.stop_event.is_set():
            if interval_ns is not None:
                ...
                # Schedule from the actual attempt start. If a backend call
                # overruns, the next attempt can begin immediately, but the
                # scheduler never emits a burst to "catch up" missed slots.
